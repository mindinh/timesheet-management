...
export function TableActionBar({
          ...
          return (
            <div key={action.id} className="flex items-center">
              ...
              >
                ...
                {action.showLabel !== false && action.labelKey && action.size !== 'icon' && <span>{label}</span>}
              </Button>
            </div>
          );
        })}
  ...
  // Legacy mode
  return (
    <div className={`flex items-center gap-2 py-2 ${align === 'right' ? 'justify-end' : 'justify-start'}`}>
      {onCreate && (
