    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md text-center">
        <DialogHeader className="items-center gap-3">
          <div
            className={cn('mx-auto flex h-12 w-12 items-center justify-center rounded-full', config.iconContainerClass)}
          >
        ...
        {variant !== 'loading' && (
          <DialogFooter className="sm:justify-center gap-3 pt-2">
            {secondaryLabel && (
              <Button variant="outline" onClick={handleSecondary} className="min-w-[100px]">
                {secondaryLabel}
              </Button>
            )}
