 ...
 */
export function ColumnSettingsDialog({ open, onClose, onApply, columns: initialColumns }: ColumnSettingsDialogProps) {
  const [columns, setColumns] = useState<ColumnSetting[]>(() => initialColumns.map((col) => ({ ...col })));
      ...
      <DialogContent hideCloseButton className="sm:max-w-[500px] flex flex-col p-0 gap-0 overflow-hidden max-h-[80vh]">
        ...
        <div className="overflow-y-auto flex-1 pb-4 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-full">
          <Table>
            ...
            <TableBody>
              {filteredColumns.map((column) => {
                ...
                return (
                  <TableRow key={column.key} className={column.visible ? 'bg-primary/5' : ''}>
                    <TableCell className="pl-6">
                      <Checkbox checked={column.visible} onCheckedChange={() => handleToggleVisibility(column.key)} />
                    </TableCell>
                    <TableCell>
                      <span
                        className={`text-sm ${column.visible ? 'text-foreground font-medium' : 'text-muted-foreground'}`}
                      >
                        {column.label}
                      </span>
                    </TableCell>
          ...
          {filteredColumns.length === 0 && (
            <div className="py-8 text-center text-muted-foreground">No columns found matching your search.</div>
          )}
        </div>

        {/* Footer */}
        <DialogFooter className="p-4 border-t border-border flex-shrink-0 bg-background sm:justify-end">
          <Button variant="create" onClick={handleOk} className="rounded-xl min-w-[80px]">
            OK
          </Button>
