 ...
 */
export function FilterSettingsDialog({ open, onClose, onApply, filters: initialFilters }: FilterSettingsDialogProps) {
  const [filters, setFilters] = useState<FilterSetting[]>(() => initialFilters.map((col) => ({ ...col })));
  ...
  const handleToggleVisibility = useCallback((key: string) => {
    setFilters((prev) => prev.map((col) => (col.key === key ? { ...col, visible: !col.visible } : col)));
  }, []);
      ...
      <DialogContent hideCloseButton className="sm:max-w-[500px] flex flex-col p-0 gap-0 overflow-hidden max-h-[80vh]">
        ...
        <div className="overflow-y-auto flex-1 pb-4 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-thumb]:rounded-full">
          <Table>
            ...
            <TableBody>
              {filteredFilters.map((filter) => {
                ...
                return (
                  <TableRow key={filter.key} className={filter.visible ? 'bg-primary/5' : ''}>
                    <TableCell className="pl-6">
                      <Checkbox checked={filter.visible} onCheckedChange={() => handleToggleVisibility(filter.key)} />
                    </TableCell>
                    <TableCell>
                      <span
                        className={`text-sm ${filter.visible ? 'text-foreground font-medium' : 'text-muted-foreground'}`}
                      >
                        {filter.label}
                      </span>
                    </TableCell>
          ...
          {filteredFilters.length === 0 && (
            <div className="py-8 text-center text-muted-foreground">No filters found matching your search.</div>
          )}
        </div>

        {/* Footer */}
        <DialogFooter className="p-4 border-t border-border flex-shrink-0 bg-background sm:justify-end">
          <Button variant="default" onClick={handleOk} className="rounded-xl min-w-[80px]">
            OK
          </Button>
          <Button variant="outline" onClick={onClose} className="rounded-xl min-w-[80px]">
            Cancel
          </Button>
        </DialogFooter>
      </DialogContent>
