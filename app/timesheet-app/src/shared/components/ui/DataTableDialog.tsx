...
export function DataTableDialog<T = any>({
  ...
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set(initialSelection.map(getRowId)));
  ...
  const handleSelectionChange = useCallback(
    (newSelectedIds: Set<string>) => {
      ...
      const newSelectedItems = data.filter((item) => newSelectedIds.has(getRowId(item)));
          ...
          {showSearch && (
            <div className="flex gap-2">
              ...
              <Button onClick={handleSearch} disabled={isLoading}>
                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : t('common.search', 'Search')}
              </Button>
            </div>
          )}
          ...
          {selectionMode === 'multiple' && selectedItems.length > 0 && (
            <div className="flex flex-wrap gap-2 p-3 bg-muted rounded-lg border">
              ...
              {selectedItems.map((item) => {
                ...
                return (
                  <Badge key={id} variant="outline" className="flex items-center gap-1 text-sm px-2 py-1 bg-background">
                    {getDisplayName(item)}
                    <X className="w-3 h-3 cursor-pointer hover:text-primary" onClick={() => handleRemoveSelected(id)} />
                  </Badge>
                );
              })}
            </div>
