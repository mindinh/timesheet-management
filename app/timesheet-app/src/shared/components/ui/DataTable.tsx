 ...
 */
function DefaultCellRenderer<T>({ column, value, row }: { column: DataTableColumn<T>; value: unknown; row: T }) {
  ...
  switch (column.renderType) {
    case 'link':
      return <span className={column.className || 'text-primary font-medium'}>{value as React.ReactNode}</span>;
    ...
    case 'date':
      return <span className={column.className}>{value ? formatDate(value as string) : '-'}</span>;

    case 'number':
      return <span className={column.className || 'text-sm text-foreground'}>{(value as React.ReactNode) ?? 0}</span>;
    ...
    case 'text':
    default:
      return <span className={column.className}>{(value as React.ReactNode) ?? '-'}</span>;
  }
}
...
export function DataTable<T = any>({
  ...
  return (
    <div
      className={`bg-card rounded-xl shadow-sm border-2 border-border hover:border-[var(--color-primary)] transition-all overflow-hidden ${className}`}
    >
      {/* Title */}
      {title && (
