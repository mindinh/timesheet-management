 ...
 */
export function MultiSelectFilter({ config, value, onChange }: FilterComponentProps<MultiSelectFilterConfig>) {
  ...
  // Load options from async loader if provided
  useEffect(() => {
    if (config.optionsLoader && !config.options) {
      setIsLoading(true);
      config
        .optionsLoader()
        .then(setOptions)
      ...
      <PopoverTrigger asChild>
        <div
          ...
          className={cn(
            ...
            isOpen || isExpanded ? 'border-2 border-[var(--color-brand)]' : 'hover:border-[var(--input-border-hover)]',
        ...
        {config.showSelectAll !== false && (
          <div className="p-2 border-b border-border">
            <div className="flex items-center justify-between">
              <Button variant="ghost" size="sm" onClick={handleSelectAll} className="h-7 text-sm">
                {t('filterbar.selectAll', 'Select All')}
              </Button>
              <Button variant="ghost" size="sm" onClick={handleClearAll} className="h-7 text-sm">
