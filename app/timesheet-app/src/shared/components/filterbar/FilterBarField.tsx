...
export function FilterBarField({ config, value, onChange, overlay }: FilterBarFieldProps) {
  ...
  const colSpanClass =
    config.colSpan && config.colSpan > 1 ? `col-span-${config.colSpan}` : '';

  return (
    <div className={`space-y-1.5 flex flex-col min-w-0 ${colSpanClass}`}>
...
function RenderFilterControl(
  ...
  switch (config.type) {
    ...
    case 'valueHelp':
      return <ValueHelpFilter config={config} value={value} onChange={onChange} />;
