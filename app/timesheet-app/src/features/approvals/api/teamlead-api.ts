...
export interface TimesheetBatch {
  ...
  totalHours?: number;
  createdAt?: string;
  teamLead?: { id: string; firstName: string; lastName: string };
}

export async function getAllBatches(): Promise<TimesheetBatch[]> {
  const data: unknown = await api.get(TEAMLEAD_URL.batches, {
    $expand: 'timesheets($expand=user),teamLead',
  ...
  return list.map((b) => ({
    ...
    timesheetCount: Array.isArray(b.timesheets) ? b.timesheets.length : undefined,
    totalHours: Array.isArray(b.timesheets)
      ? b.timesheets.reduce((sum: number, ts: any) => sum + (Number(ts.totalHours) || 0), 0)
      : undefined,
    createdAt: b.createdAt as string | undefined,
    teamLead: b.teamLead
      ? { id: String(b.teamLead.ID || b.teamLead.id), firstName: String(b.teamLead.firstName), lastName: String(b.teamLead.lastName) }
      : undefined,
  }));
}

export async function getBatchesByPeriod(month: number, year: number): Promise<TimesheetBatch[]> {
  const data: unknown = await api.get(TEAMLEAD_URL.batches, {
...
export async function getBatchTimesheets(batchId: string): Promise<Timesheet[]> {
  const data: unknown = await api.get(TEAMLEAD_URL.timesheets, {
  ...
  return list.map((ts) => ({
    ...
    user: ts.user
      ? {
          id: String((ts.user as any).ID || (ts.user as any).id),
          firstName: String((ts.user as any).firstName),
          lastName: String((ts.user as any).lastName),
          email: String((ts.user as any).email),
          role: String((ts.user as any).role),
        }
      : undefined,
  }));
}
...
export async function reviewEntryByTeamLead(
  entryId: string,
  status: string,
...
export async function createBatch(timesheetIds: string[]): Promise<string> {
  const data: unknown = await api.post(TEAMLEAD_URL.createBatch, { timesheetIds });
  return String(data && typeof data === 'object' && 'value' in data ? (data as any).value : data);
}

export async function getAdmins(): Promise<{ id: string; firstName: string; lastName: string; role: string }[]> {
  const data: unknown = await api.get('/user/Users', {
    $filter: `role eq 'Admin'`,
    $select: 'ID,firstName,lastName,role',
  });
  const list = (data && typeof data === 'object' && 'value' in data ? (data as any).value : data) as Record<string, any>[];
  return (list || []).map((u) => ({
    id: String(u.ID || u.id),
