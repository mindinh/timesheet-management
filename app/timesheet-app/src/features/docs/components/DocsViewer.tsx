  ...
  return (
    <article className="prose prose-sm max-w-none">
      ...
      <Markdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold text-foreground mt-8 mb-4 pb-2 border-b border-border">{children}</h1>
          ),
          ...
          ul: ({ children }) => (
            <ul className="list-disc list-inside space-y-1 mb-4 text-sm text-foreground/90">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="list-decimal list-inside space-y-1 mb-4 text-sm text-foreground/90">{children}</ol>
          ),
          ...
          pre: ({ children }: React.DetailedHTMLProps<React.HTMLAttributes<HTMLPreElement>, HTMLPreElement>) => {
            ...
            return <pre className="bg-muted rounded-lg p-4 overflow-x-auto mb-4 border border-border">{children}</pre>;
          },
          blockquote: ({ children }) => (
            <blockquote className="border-l-4 border-primary/40 pl-4 py-1 my-4 bg-primary/5 rounded-r-lg text-sm text-foreground/80">
              {children}
            </blockquote>
          ),
          table: ({ children }) => (
