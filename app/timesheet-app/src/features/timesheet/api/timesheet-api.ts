...
import type { Timesheet, TimesheetEntry, TimesheetStatusType, ApprovalHistory } from '@/shared/types';
...
export async function getTimesheetByMonth(month: number, year: number, userId: string): Promise<Timesheet | null> {
  const data: unknown = await api.get(TIMESHEET_URL.timesheets, {
...
export async function getTimesheetDetail(timesheetId: string): Promise<Timesheet & { entries: TimesheetEntry[] }> {
...
export async function getMyTeamLead(): Promise<{
  id: string;
  firstName: string;
  lastName: string;
  email: string;
...
export async function getTeamLeads(): Promise<{ id: string; firstName: string; lastName: string; email: string }[]> {
  try {
    const data: unknown = await api.get(TIMESHEET_URL.getTeamLeads);
    const list = data && typeof data === 'object' && 'value' in data ? (data as any).value : data;
...
export async function submitTimesheet(timesheetId: string, teamLeadId?: string): Promise<string> {
  const data: unknown = await api.post(TIMESHEET_URL.submit, { timesheetId, ...(teamLeadId ? { teamLeadId } : {}) });
...
export async function exportToExcel(timesheetId: string): Promise<void> {
  const result: unknown = await api.post(TIMESHEET_URL.exportToExcel, { timesheetId }, { responseType: 'blob' });
