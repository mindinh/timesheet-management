...
interface TimesheetState {
  ...
  submitTimesheet: (year: number, month: number, teamLeadId?: string) => Promise<void>;
}

// Known dev users from package.json mocked auth – used as initial state so
// the UI never shows"Loading user..." while waiting for the backend.
const DEV_USERS: User[] = [
  {
    id: 'bdad7628-9dd7-46ea-b45b-7b37f1f00b68',
    firstName: 'Nam',
    lastName: 'Nguyen',
    role: 'Employee',
  ...
  {
    id: '2b7a2d96-0e94-4d13-8a03-7f8a70562590',
    firstName: 'Alice',
    lastName: 'Nguyen',
    role: 'Employee',
  ...
  {
    id: 'c4e8f1a2-3b56-4d78-9e01-a1b2c3d4e5f6',
    firstName: 'Bob',
    lastName: 'Tran',
    role: 'TeamLead',
  ...
  {
    id: 'd5f902b3-4c67-5e89-af12-b2c3d4e5f6a7',
    firstName: 'Charlie',
    lastName: 'Le',
    role: 'TeamLead',
  ...
  {
    id: 'e6a003c4-5d78-6f90-b023-c3d4e5f6a7b8',
    firstName: 'Diana',
    lastName: 'Pham',
    role: 'Admin',
...
// Default: start as Diana (Admin) so all pages are immediately accessible
const DEFAULT_USER = DEV_USERS[4];
setMockUserId(DEFAULT_USER.id);
...
export const useTimesheetStore = create<TimesheetState>((set, get) => ({
  ...
  fetchAvailableUsers: async () => {
    try {
      ...
      if (users.length > 0) set({ availableUsers: users });
    } catch (error) {
  ...
  submitTimesheet: async (year, month, teamLeadId) => {
    ...
    try {
      ...
      await submitTimesheetApi(timesheetId, teamLeadId);
