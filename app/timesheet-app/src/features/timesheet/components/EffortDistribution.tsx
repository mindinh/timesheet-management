...
export function EffortDistribution({ entries, projects }: EffortDistributionProps) {
  ...
  const colors = ['bg-[var(--chart-1)]', 'bg-[var(--chart-2)]', 'bg-[var(--chart-5)]'];
  ...
  return (
    <div className="p-5 bg-card rounded-lg border shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Effort Distribution</h3>
        ...
        {projectData.map((project, index) => (
          <div key={project.projectId} className="flex-1 min-w-0">
            <div className="flex items-center justify-between text-sm mb-1.5">
              ...
              <span className="text-muted-foreground ml-2">{project.percentage}%</span>
            </div>
