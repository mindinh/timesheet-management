...
export async function fetchTimesheetBatchById(id: string): Promise<TimesheetBatchDetail> {
  const data: unknown = await api.get(`${ADMIN_URL.timesheetBatches}('${id}')`, {
    $expand:
      'teamLead($select=ID,firstName,lastName,email),history($expand=actor($select=ID,firstName,lastName);$orderby=timestamp desc),timesheets($expand=user($select=ID,firstName,lastName),entries($expand=project,task),approvalHistory($expand=actor($select=ID,firstName,lastName);$orderby=timestamp desc))',
  });
