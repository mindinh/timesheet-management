...
import {
  fetchTimesheetBatchById,
  markBatchDoneApi,
  rejectBatchApi,
  adminModifyEntryHours,
  getAdminTimesheetDetail,
  type TimesheetBatchDetail,
} from '../api/admin-api';
...
export default function AdminBatchDetailPage() {
  ...
  const handleMarkDone = () => {
    setConfirmDialog({
      ...
      description:
        'Are you sure you want to finalize this batch? All timesheets within will be marked as Finished and become read-only.',
      onConfirm: async () => {
        try {
          ...
          setStatusDialog({
            ...
            description: 'Batch marked as Done successfully.',
          });
          loadBatch();
        } catch (error: unknown) {
  ...
  const executeReject = async () => {
    if (!rejectComment.trim()) {
      setStatusDialog({
        ...
        variant: 'warning',
        title: 'Validation',
        description: 'Please provide a reason for rejecting this batch.',
      });
      return;
    }
    ...
    try {
      ...
      setStatusDialog({
        open: true,
        variant: 'success',
        title: 'Batch Reopened',
        description: 'Batch rejected and returned to submitted state for Team Lead.',
      });
      setRejectComment('');
      loadBatch();
    } catch (error: unknown) {
  ...
  const executeRejectTimesheet = async () => {
    ...
    try {
      ...
      setStatusDialog({
        ...
        description: 'Timesheet has been rejected individually.',
      });
      setTsRejectComment('');
      setTsRejectId(null);
  ...
  const getEntryColumns = (ts: any): DataTableColumn<TimesheetEntry>[] => [
    ...
    },
    {
      ...
      labelKey: 'Logged',
      width: 80,
      render: (_, entry) => <span className="text-muted-foreground text-right block w-full">{entry.hours}</span>,
    },
    {
      key: 'approved',
      labelKey: 'Approved',
      ...
      render: (_, entry) => {
        ...
        return (
          <div
            className={` inline-flex items-center justify-end gap-2 cursor-pointer group hover:bg-muted/50 px-2 py-1 rounded transition-colors w-full ${status !== 'Pending' ? 'pointer-events-none' : ''}`}
          ...
          >
            {entry.approvedHours !== undefined && entry.approvedHours !== entry.hours ? (
              <span
                className="text-emerald-600 dark:text-emerald-400 font-medium bg-emerald-50 dark:bg-emerald-900/20 px-1 rounded"
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-xl font-bold text-primary">Batch Details</h1>
              <span
            ...
            <p className="text-sm text-muted-foreground bg-muted/50 px-2 py-1 rounded inline-block">ID: {batchId}</p>
          </div>
              ...
              {history.map((log) => (
                <div key={log.ID} className="flex gap-4">
                  ...
                  <div className="flex-1 pb-4">
                    ...
                    {log.comment && (
                      <p className="text-sm text-muted-foreground mt-1 bg-muted/40 p-2 rounded-md">"{log.comment}"</p>
                    )}
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="p-6 border-b border-border bg-muted/20">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Timesheets approved by the Team Lead and belonging to this batch.
          </p>
        </div>
        {timesheets.length === 0 ? (
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          Timesheet ID: <span className="text-xs">{ts.ID.split('-')[0]}...</span>
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    {loadingDetails[ts.ID] ? (
                      ...
                      <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
                        ...
                        {timesheetDetails[ts.ID].entries.length === 0 ? (
                          <p className="text-sm text-muted-foreground py-2">No entries found for this timesheet.</p>
                        ) : (
                        ...
                        <div className="flex justify-end pr-4 text-sm">
                          ...
                          <span className="font-bold">{timesheetDetails[ts.ID].totalHours}</span>
                        </div>
                        ...
                        {timesheetDetails[ts.ID].comment && (
                          <div className="mt-4 p-3 bg-muted/40 rounded-md border border-border/50 text-sm">
                            ...
                            <span className="text-muted-foreground">"{timesheetDetails[ts.ID].comment}"</span>
                          </div>
                        )}
                              ...
                              {ts.approvalHistory.map((log) => (
                                <div key={log.ID} className="flex gap-3 text-sm border-l-2 border-muted pl-3 pb-1">
                                  <div className="flex-1">
                                    ...
                                    {log.comment && <p className="text-muted-foreground mt-1 text-xs">{log.comment}</p>}
                                  </div>
                                </div>
                              ))}
                            </div>
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Rejecting this batch will return all its timesheets to the"Submitted" state, sending them back to the Team
              Lead for review.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to reject this specific timesheet. It will be sent back to the employee, and removed from
              this batch's final approval.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
