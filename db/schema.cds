...
type ProjectType     : String(20) enum {
  Papierkram;
  Internal;
  External;
  Others;
...
entity User : cuid, managed {
  ...
  directReports : Association to many User
                    on directReports.manager = $self;
  timesheets    : Association to many Timesheet
                    on timesheets.user = $self;
}
...
entity TimesheetBatch : cuid, managed {
  ...
  history    : Composition of many BatchHistory
                 on history.batch = $self;
}
...
entity Timesheet : cuid, managed {
  ...
  comment         : String(1000);
  totalHours      : Decimal(7, 2); // Calculated automatically on submit/modify
  mainDays        : Decimal(5, 2); // Calculated automatically (totalHours / 8)
  entries         : Composition of many TimesheetEntry
                      on entries.timesheet = $self;
  approvalHistory : Composition of many ApprovalHistory
                      on approvalHistory.timesheet = $self;
}
