...
export class TimesheetImportHandler {
  ...
  private async onImportFromExcel(req: any) {
    ...
    if (ts.status !== 'Draft' && ts.status !== 'Reopened') {
      return req.reject(
        400,
        `Cannot import – timesheet is "${ts.status}". Only Draft or Reopened timesheets can be edited.`
      );
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (projectRef) {
        const matched = allProjects.find(
          (p: any) =>
            p.name?.toLowerCase() === projectRef.toLowerCase() || p.code?.toLowerCase() === projectRef.toLowerCase()
        );
        if (matched) {
    ...
    return rowErrors.length ? `${msg} Skipped rows: ${rowErrors.join(' | ')}` : msg;
  }
}
