...
export class TimesheetBulkHandler {
  ...
  private async onBulkSubmitToAdmin(req: any) {
    ...
    if (user.role !== 'TeamLead' && user.role !== 'Admin') {
      return req.reject(403, 'Only Team Leads or Admins can bulk-submit timesheets');
    }
    ...
    // Validate all timesheets are Approved and have an existing batch
    ...
    for (const timesheetId of timesheetIds) {
      ...
      if (!ts.batch_ID) {
        errors.push(`${timesheetId}: no batch found. Timesheet must be re-submitted by employee.`);
        continue;
      }
    ...
    // Group by existing batch_ID – update each batch once
    const batchIdsToUpdate = new Set<string>(validTimesheets.map((ts) => ts.batch_ID));
    ...
    for (const batchId of batchIdsToUpdate) {
      ...
      await INSERT.into(BatchHistory).entries({
        ...
        status: 'SubmittedToAdmin',
        comment: `Batch submitted to Admin by Team Lead`,
        timestamp: now,
      });
    }

    // Update each timesheet's currentApprover to Admin
