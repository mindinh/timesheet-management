...
export class AdminBatchHandler {
  ...
  private async onMarkBatchDone(req: any) {
    ...
    await UPDATE(Timesheet)
      .set({ status: 'Finished', finishedDate: now, comment: 'Batch marked as done by admin' })
    ...
    await UPDATE(TimesheetBatch).set({ status: 'Done' }).where({ ID: batchId });
  ...
  private async onRejectBatch(req: any) {
    ...
    if (timesheets.length === 0) return req.reject(400, 'No timesheets found in this batch');

    const now = new Date().toISOString();

    await UPDATE(Timesheet).set({ status: 'Reopened', comment }).where({ batch_ID: batchId });
    ...
    if (historyEntries.length > 0) await INSERT.into(ApprovalHistory).entries(historyEntries);
