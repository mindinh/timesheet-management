...
export class AdminExportHandler {
  ...
  private async onAdminExportToExcel(req: any) {
    ...
    // ── Fetch entries ──────────────────────────────────────────────────────
    let entryQuery = SELECT.from(TimesheetEntry).where(`date >= '${fromDate}' and date <= '${toDate}'`);
    ...
    const excelUser = filterUserId
      ? userMap[filterUserId]
      : { firstName: 'Admin', lastName: 'User', email: 'admin@conarum.com' };
