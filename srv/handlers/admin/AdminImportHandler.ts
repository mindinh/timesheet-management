...
export class AdminImportHandler {
    ...
    private async onClearDatabase(req: any) {
        ...
        try {
            ...
            const {
                ExportLog, AuditLog, BatchHistory, ApprovalHistory,
                TimesheetEntry, Timesheet, TimesheetBatch,
                Task, Project, User
    ...
    private async onRunReport(req: any) {
        ...
        try {
            ...
            const {
                User, Project, Task,
                TimesheetBatch, Timesheet, TimesheetEntry,
                ApprovalHistory, BatchHistory, AuditLog, ExportLog
            } = db.entities('sap.timesheet')

            const summary: Record<string, number> = {
                users: 0, projects: 0, tasks: 0,
                batches: 0, timesheets: 0, entries: 0,
                approvalHistory: 0, batchHistory: 0, auditLog: 0, exportLog: 0
            }
            ...
            if (userSheet) {
                ...
                for (const row of rows) {
                    if (row.manager_email && userMap.has(row.email) && userMap.has(row.manager_email)) {
                        await UPDATE(User).set({ manager_ID: userMap.get(row.manager_email) }).where({ ID: userMap.get(row.email) })
                    }
                }
            } else {
            ...
            if (batchSheet) {
                ...
                for (const row of rows) {
                    ...
                    if (!teamLeadId || !adminId) continue
            ...
            if (timesheetSheet) {
                ...
                for (const row of rows) {
                    ...
                    const existing = await SELECT.one.from(Timesheet).where({ user_ID: userId, month: row.month, year: row.year })
                ...
                for (const row of rows) {
                    ...
                    if (row.timesheet_user_email && row.timesheet_month && row.timesheet_year) {
                        ...
                        if (!timesheetId) {
                            ...
                            if (tsUserId) {
                                const ts = await SELECT.one.from(Timesheet).where({ user_ID: tsUserId, month: row.timesheet_month, year: row.timesheet_year })
                                timesheetId = ts?.ID
                            }
                        }
                    }
