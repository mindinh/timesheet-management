...
async function getVietnamPublicHolidays(year: number): Promise<Set<string>> {
  ...
  try {
    ...
    console.error(`Failed to fetch VN public holidays for ${year} (will skip OT holiday detection)`, e);
  }
...
export class AdminDashboardHandler {
  ...
  private async onGetDashboardStats(req: any) {
    ...
    for (const log of batchLogs) {
      const actor = (allUsersMap.get(log.actor_ID) || { firstName: 'System', lastName: '' }) as {
        firstName: string;
        lastName: string;
      };
      recentActivity.push({
    ...
    const projectHoursChart = Array.from(projectHoursMap.entries())
      .map(([id, hours]) => ({
    ...
    const topEmployeesChart = Array.from(topEmployeesMap.entries())
      .map(([userId, hours]) => {
